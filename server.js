...
// Génère un ID court de 6 caractères alphanumériques
function shortId() {
...
app.post('/pages', async (req, res) => {
  ...
  const payload = {
    ...
    reactivity: (metrics?.transformation_color === 'red') ? 'slow' : 'normal',
...
const SCENARIOS = {
  privateaser_search: { icon: '🔍', label: 'Visibilité Privateaser', tone: 'positive', tagLabel: 'Opportunité ✨',
  ...
  transformation: { icon: '✅', label: 'Transformation des demandes', tone: 'neutral', tagLabel: 'Recommandation',
    context: 'La majorité de vos demandes ne se transforment pas en réservations confirmées.',
    reco_slow: "Pour améliorer votre taux de transformation : répondez aux demandes rapidement, proposez des conditions claires et cohérentes avec votre vitrine, et adoptez une posture commerciale dans vos échanges. Votre chargé de compte peut vous accompagner.",
    reco_normal: "Pour améliorer votre taux de transformation : proposez des conditions simples et cohérentes avec votre vitrine, adoptez une posture commerciale et soyez assidu dans vos relances. Votre chargé de compte peut vous conseiller." },
  conversion: { icon: '🎯', label: 'Conversion de votre vitrine', tone: 'neutral', tagLabel: 'Recommandation',
  ...
  direct_traffic: { icon: '📥', label: 'Trafic via Joy', tone: 'neutral', tagLabel: 'Recommandation',
  ...
  feature_usage: { icon: '⚙️', label: 'Fonctionnalités Joy', tone: 'neutral', tagLabel: 'Recommandation',
...
const TONE_STYLE = {
  positive: { borderLeft: '4px solid #3B82F6', iconBg: 'bg-blue-50',   tag: 'bg-blue-50 text-blue-700 border-blue-200' },
  neutral:  { borderLeft: '4px solid #F0967A', iconBg: 'bg-orange-50', tag: 'bg-orange-50 text-[#F0967A] border-orange-200' },
};
...
function renderCard(key, data, index) {
  ...
  const reco = key === 'transformation'
    ? (data.reactivity === 'slow' ? s.reco_slow : s.reco_normal)
    : s.reco;
  const delay = index * 0.08;

  return `<div class="bg-white rounded-2xl px-5 py-4" style="animation:fadeIn .35s ease ${delay}s both;box-shadow:0 2px 16px rgba(26,26,94,0.07);border:1px solid #f1f5f9;border-left:${style.borderLeft}">
    <div class="flex items-center justify-between mb-3">
      <div class="flex items-center gap-2.5">
        ...
        <p class="font-bold text-[#1A1A5E] text-sm leading-tight">${s.label}</p>
      </div>
      <span class="text-xs font-semibold px-2.5 py-0.5 rounded-full border flex-shrink-0 ${style.tag}">${s.tagLabel}</span>
    </div>
    <p class="text-[#9099B0] text-xs leading-relaxed mb-2 border-t border-slate-100 pt-3 italic">${s.context}</p>
    <div class="bg-slate-50 rounded-xl px-3 py-2.5 mb-3">
      <p class="text-[#1A1A5E] text-xs leading-relaxed font-medium">${reco}</p>
    </div>
    <button onclick="window.open('https://joy.privateaser.com','_blank')"
      class="inline-flex items-center justify-center gap-2 text-white font-bold px-4 py-2.5 rounded-xl text-xs w-full" style="background:#F0967A">
  ...
  <script>
    tailwind.config = {
      theme: { extend: {
        ...
        colors: { joy: { navy:'#1A1A5E', muted:'#9099B0', bg:'#F8F8FF' } },
...
<body class="text-joy-navy min-h-screen">
  <header class="bg-white border-b border-slate-100 px-5 py-3 flex items-center justify-between sticky top-0 z-10">
    ${LOGO_SVG}
    <span class="text-xs font-semibold text-joy-muted truncate max-w-[180px]">${name}</span>
  </header>
  <main class="max-w-lg mx-auto px-4 py-4 space-y-3 pb-8">
    <section class="bg-white rounded-2xl shadow-card border border-slate-100 px-5 py-4">
      <p class="text-[#F0967A] text-xs font-bold uppercase tracking-widest mb-1">Vos recommandations Joy</p>
      <p class="text-joy-muted text-xs leading-relaxed">${subtitle}</p>
    </section>
